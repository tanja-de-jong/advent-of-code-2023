...
def findType(hand):
	...
	occurrences = sorted(charCount.values())
	if 'J' in charCount:
		occurrences.remove(charCount['J'])
		if occurrences:
			occurrences[-1] += charCount['J']
		else:
			occurrences.append(5)
	occurrences = list(filter(lambda a: a != 1, occurrences))

	for i in range(0, len(rankings)):
		if occurrences == sorted(rankings[i]):
			return i
	return len(rankings)
